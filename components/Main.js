import styles from "../styles/Main.module.scss";

export default function Main() {
  return (
    <>
      <main className={styles.main}>
        <h2>About Me</h2>
        <p>
          Front-end Web Developer position with professional experience at a design agency in Vancouver, Canada.
        </p>

        <h2>What I can</h2>
        <ul>
          <li>HTML5</li>
          <li>CSS3</li>
          <li>JavaScript</li>
          <li>React.js</li>
          <li>Next.js</li>
          <li>WordPress</li>
          <li>Git</li>
          <li>Photoshop</li>
          <li>Illustrator</li>
        </ul>

        <h2>Projects</h2>
        <ul>
          <li>BudgeTicket - Flight Tickets Search App</li>
          <li>KUMONOS Corporation - Corporate Website</li>
          <li>Portfolio 2020 (Gastby.js)</li>
        </ul>

        <h2>Experiences</h2>
        <ul>
          <li>Web Developer, New World Enterprises (Sep. 2020 - present)</li>
          <li>Junior Web Developer, Signals Design Group (Sep. 2019 - Mar. 2020)</li>
          <li>Website Manager, Kumonos Corporation (Apr. 2015 - Aug. 2018)</li>
        </ul>

        <h2>Educations</h2>
        <ul>
          <li>Bachelor of Engineering, Architecture and Environmental Design, Osaka Sangyo University (2010 - 2015)</li>
          <li>Certificate, New Media Design and Web Development, British Columbia Institute of Technology (2018 - 2019)</li>
        </ul>
      </main>
    </>
  )
}
